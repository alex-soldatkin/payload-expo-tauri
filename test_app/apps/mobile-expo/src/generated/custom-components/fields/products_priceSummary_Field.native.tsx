...
import { useField } from '@payload-universal/ui'
    ...
    return (
        ...
        >
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                <Text style={{ fontWeight: 600, fontSize: 14 }}>Price Summary</Text>
                {discount !== null && (
                    <Text style={{ color: '#1a7f37', fontSize: 12, fontWeight: 600 }}>
                        {discount}% off
                    </Text>
                )}
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 2 }}>
                <Text style={{ color: '#666', fontSize: 13 }}>Base price</Text>
                <Text style={{ fontSize: 13 }}>{formatMoney(price)}</Text>
            </View>
            {onSale && (
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 2 }}>
                    <Text style={{ color: '#666', fontSize: 13 }}>Sale price</Text>
                    <Text style={{ fontSize: 13 }}>{formatMoney(salePrice)}</Text>
                </View>
            )}
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 2 }}>
                ...
                <Text style={{ fontSize: 13 }}>{formatMoney(tax)}</Text>
            </View>
            <View
                style={{
                    flexDirection: 'row',
            ...
            >
                <Text style={{ fontWeight: 600, fontSize: 13 }}>Total</Text>
                <Text style={{ fontWeight: 600, fontSize: 13 }}>{formatMoney(total)}</Text>
            </View>
        </View>
    )
}
