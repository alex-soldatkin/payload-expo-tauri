/**
 * useGanttDraft — buffered draft + source-editor state machine for the
 * GanttCustomizeSheet. All changes buffer locally; handleSave flushes the
 * draft via onSave (rowSort passes through unchanged so preset round-trips
 * keep it). Pure state/logic — no JSX.
 */
...
import type { Draft, SourceEditorState } from '../types'
import { ensureUniqueIds, nextPaletteColor } from '../utils'

export type UseGanttDraftArgs = {
  visible: boolean
