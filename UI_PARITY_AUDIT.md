...
| Gantt view | ❌ (no web equivalent) | ✅ **Native-first extension beyond web parity (Phase 30)**. Fourth view-mode for any collection with ≥1 date field (same `collectionHasCalendarDateFields` eligibility gate as calendar; entry hidden otherwise). `admin-native/src/gantt/` — infinite-scroll both axes: horizontal `ScrollView` [sticky `TimeAxis` (month band, day ticks, weekend shading, today accent) above a vertical `FlatList` of lane-packed row tracks] at full timeline width; frozen 150pt glass title column absolutely overlaid with `translateY` synced via `Animated.event` (native driver). Infinite time window `[startKey, endKey]`: init = min(event starts, today)−14d … max(event ends, today)+30d clamped ±366d auto; edge-proximity (<7 cols) extends by 60d; left-extension is jump-free via `contentOffset` compensation in `onContentSizeChange` (pixel delta = new width − old width). FlatList windowed natively via `getItemLayout` from pure row heights. Gestures PanResponder-only (per Phase 29 conventions): HANDLE RESIZE — dedicated 20pt zones at bar ends, no long-press; BODY SHIFT — 200ms Pressable `onLongPress` ARMS, bar-root PanResponder claims first move in capture phase (kanban hand-off); STATIC HOLD → PEEK — armed hold released <8pt travel funnels to `onPreviewDoc`; `finishDrag` IDEMPOTENT across all three release paths. Point-source diamonds (no `endField`) are shiftable but NOT resizable (edge responders guard on `bar.point`). Day-snapped commits emit ISO datetimes preserving wall-clock time (`shiftIsoByDays` via `setDate`); inversion-guarded by `computeNextRange`. All event/date/scale math from shared `../scheduling` — zero duplication. `GanttCustomizeSheet`: Sortable reorder, native Toggle visibility outside Sortable, ColorPicker; ZOOM S/M/L → 16/28/44 px-per-day. Presets: `ganttSources` + `ganttOptions.{pxPerDay,rowSort}` round-trip matches server `ViewPresets` json fields; `presetToGanttConfig` restores from a stored preset. `onUpdateDates` dot-path patch: nested paths rebuild whole root key (RxDB `incrementalPatch` merges top-level keys only) seeded from the doc + patch-so-far so start+end under one nested group compose without overwriting each other. |
...
- **Jump-free window extension**: trigger the left edge extension (scroll past the guard threshold) — the frozen title column and time axis stay in sync with the row list; no flicker or position jump.
- **Frozen title column sync**: `translateY` via `Animated.event` (native driver) stays in sync with the FlatList vertical scroll position, including rubber-band overscroll; title column never drifts.
- **Handle drags vs FlatList/ScrollView scroll claim**: horizontal drag on a bar handle claims the touch (|dx| > 1.4|dy|) without hijacking vertical FlatList scroll or horizontal ScrollView pan on non-horizontal moves; a vertical swipe starting over a handle does NOT start a resize.
- **Body shift (200ms hold → move)**: long-press arms the drag; the FIRST horizontal move after arming is captured in the correct phase; scroll locks on both axes; day-snapped ghost + tooltip updates per crossed column; release fires `onUpdateDates` with correct ISO datetimes; failure springs the bar back (no optimistic ghost stuck on screen).
- **Static-hold peek (< 8pt travel)**: armed long-press released without moving opens the preview sheet via `onPreviewDoc`; does NOT fire `onUpdateDates`; release after exactly arming (tap-hold-release) always reaches the peek path, not the drag-commit path.
- **Idempotent finishDrag**: no double-fire of `onUpdateDates` or `onPreviewDoc` when the Pressable's `onPressOut` and the responder release race (both fire — only the first acts due to `activeRef` guard).
- **Point diamonds**: sources without `endField` render as 16pt diamonds; body-shift works; handle zones do NOT appear (edge responders bail on `bar.point`); no resize path is reachable.
- **Peek: no ScrollablePreview.Trigger on bars** — the preview sheet is a JS BottomSheet; no UIKit recognizer conflict.
- **GanttCustomizeSheet**: source reorder (Sortable dnd), ColorPicker outside Sortable tree, visibility Toggle section outside Sortable, ZOOM S/M/L → 16/28/44 px-per-day updates the chart immediately; pxPerDay persists in `gantt_config:{slug}`.
- **Presets round-trip**: save a gantt preset (sources + zoom); apply it; verify sources and pxPerDay are restored; share with another user; apply shared preset.
- **`onUpdateDates` dot-path patch**: for Events with nested start/end fields (e.g. `scheduling.scheduledPublish`/`scheduling.scheduledUnpublish`), both fields must update in a single `incrementalPatch` without one overwriting the other.
- **`readOnlyDocIds` dimming**: during an in-flight patch the bar dims and cannot be dragged again until the patch resolves.
- **Eligibility gate**: collections without date fields (Users) do NOT show the Gantt entry in the view selector; a stale persisted `'gantt'` mode degrades to table.
- **Gantt + FilterChips + FilterBottomSheet**: filtering docs in gantt mode (same `boardDocs` pipeline) updates bars correctly; rows with no bars in the window remain visible as empty tracks.
