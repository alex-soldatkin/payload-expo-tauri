 ...
 * components.
 *
 * Web admin custom components render `<button>` / Payload `<Button>` elements.
 * The native-component-codegen maps those onto this shim so buttons feel
 * native (Apple-like, liquid glass where available) instead of an
 * HTML-translated Pressable. Two tiers:
 *
 ...
 *     matchContents so the button keeps its intrinsic size and stays
 *     tappable. buttonStyle('glass' / 'glassProminent') on iOS 26+
 *     (expo-glass-effect isLiquidGlassAvailable), 'bordered' /
 *     'borderedProminent' otherwise; controlSize('regular'); tint from the
 *     variant; role 'destructive' for destructive actions.
 *
 ...
 *     border, primary-tinted label, pressed opacity. Platform-decent,
 *     never HTML-ish.
 *
