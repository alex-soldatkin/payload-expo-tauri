/**
 * useKanbanDrag — the board's pure-RN PanResponder drag machinery.
 *
 * Owns the entire drag lifecycle end-to-end (the codebase's proven gesture
 * tier: BottomSheet, SwipeToDeleteRow, Toast). RNGH/reanimated-dnd is
 * deliberately NOT used: its Gesture.Pan().activateAfterLongPress() loses the
 * native recognizer arbitration to the column FlatLists nested inside the
 * horizontal ScrollView and never activates on device.
 *
 ...
 *    drag.
 *  - HIT-TESTING: release hit-tests the overlay card's CENTER against the
 *    column frames (useColumnFrames) → handleMove (same-column drops no-op);
 *    a miss springs the overlay home and restores the card.
...
import {
  ...
  SNAP_INTERVAL,
  STATIC_PRESS_MAX_DIST,
} from '../constants'
import { useColumnFrames } from './useColumnFrames'

type UseKanbanDragArgs = {
  onPreviewCard?: (doc: KanbanDoc) => void
  handleMove: (doc: KanbanDoc, toValue: string | null) => void
}

export function useKanbanDrag({ onPreviewCard, handleMove }: UseKanbanDragArgs) {
  // ── Drag state ───────────────────────────────────────────────────────
  ...
  const { getColumnRefCallback, refreshColumnFrames, hitTestCardCenter } = useColumnFrames({
    boardOrigin,
    overlayPosRef,
    cardSizeRef,
  })
