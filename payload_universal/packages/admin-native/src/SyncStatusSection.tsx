  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={[styles.headerTitle, { color: c.text }]}>Sync Status</Text>
      ...
      {completedCount > 0 && (
        <View style={styles.completedRow}>
          <Text style={[styles.completedText, { color: c.textMuted }]}>
      ...
      {/* Retry all */}
      {errorCount > 0 && (
        <Pressable style={[styles.retryAllBtn, { borderColor: c.error }]} onPress={onRetryAllUploads}>
          <Text style={[styles.retryAllText, { color: c.error }]}>Retry All Failed</Text>
        </Pressable>
      )}

      {/* Empty state */}
      {pendingUploads.length === 0 && syncStatus === 'idle' && (
        <Text style={[styles.emptyText, { color: c.textMuted }]}>Everything is up to date</Text>
      )}
    </View>
  )
}
